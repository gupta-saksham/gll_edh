...
def family_policy(
    ...
    target_gap_share = jnp.where(
        obs.soc_headroom_kwh > 1.0e-6,
        jnp.clip((target_kwh - obs.soc_kwh) / obs.soc_headroom_kwh, 0.0, 1.0),
        0.0,
    )
    target_charge_kw = obs.bat_charge_max_kw * target_gap_share
...
def family_controller(policy_id: float = 15.0) -> Controller:
    ...
    return Controller(
        ...
        fn=family_policy,
        params={"policy_id": jnp.float32(policy_id), "voltage_enabled": jnp.float32(1.0)},
